...
from .errors import InvalidCheckin, FailedCheckin
...
class Reservation:
    ...
    def safe_request(self, url, body=None, max_attempts=MAX_ATTEMPTS):
        try:
            attempts = 0
            ...
            while True:
                ...
                if "httpStatusCode" in data and data["httpStatusCode"] in [
                    ...
                    if attempts > max_attempts:
                        raise FailedCheckin("Unable to get data, killing self")
    ...
    def load_json_page(self, url, body=None, max_attempts=MAX_ATTEMPTS):
        data = self.safe_request(url, body, max_attempts)
        if not data:
            return
